...
class ApiService {
  ...
  /// POST /register (Versi untuk Admin membuat user lain, tanpa simpan token)
  static Future<ApiResponse<void>> createAdminAccount({
    required String name,
    required String username,
    required String password,
    required String passwordConfirmation,
    ...
    try {
      ...
      throw ApiException('Gagal membuat akun admin baru.');
    }
  }
  ...
  /// PUT /layanan/{id} (Admin only)
  static Future<LayananModel> updateLayanan({
    required int id,
    required String nama,
    required int durasi,
    required String ruangan,
    ...
    try {
      ...
      final response = await http
          .put(
            Uri.parse(
                '${AppConstants.baseUrl}${AppConstants.layananEndpoint}/$id'),
            headers: headers,
            body: jsonEncode(
                {'nama': nama, 'durasi': durasi, 'ruangan': ruangan}),
          )
      ...
      throw ApiException('Gagal memperbarui layanan.');
    }
  }

  /// DELETE /layanan/{id} (Admin only)
  static Future<void> deleteLayanan(int id) async {
    try {
      ...
      throw ApiException('Gagal menghapus layanan.');
    }
  }
  ...
  /// POST /jadwal (Admin only)
  static Future<JadwalModel> createJadwal({
    required int layananId,
    ...
    try {
      ...
      final response = await http
          .post(
            Uri.parse('${AppConstants.baseUrl}${AppConstants.jadwalEndpoint}'),
            headers: headers,
            body: jsonEncode({
              'layanan_id': layananId,
              'tanggal': tanggal,
      ...
      final body = _parseResponse(response);
      return JadwalModel.fromJson(body['data'] as Map<String, dynamic>);
    } on ApiException {
      rethrow;
    } catch (e) {
      throw ApiException('Gagal membuat jadwal baru.');
    }
  }

  /// PUT /jadwal/{id} (Admin only)
  static Future<JadwalModel> updateJadwal({
    required int id,
    required int layananId,
    required String tanggal,
    required String jamMulai,
    ...
    try {
      ...
      final response = await http
          .put(
            Uri.parse(
                '${AppConstants.baseUrl}${AppConstants.jadwalEndpoint}/$id'),
            headers: headers,
            body: jsonEncode({
              'layanan_id': layananId,
              'tanggal': tanggal,
      ...
      throw ApiException('Gagal memperbarui jadwal.');
    }
  }

  /// DELETE /jadwal/{id} (Admin only)
  static Future<void> deleteJadwal(int id) async {
    try {
      ...
      throw ApiException('Gagal menghapus jadwal.');
    }
  }
  ...
  static Future<ApiResponse<Map<String, dynamic>>> checkStatus() async {
    try {
      ...
      final response = await http
          .get(
            Uri.parse(
                '${AppConstants.baseUrl}${AppConstants.antrianEndpoint}/status'),
            headers: headers,
          )
          .timeout(const Duration(seconds: 15));
  ...
  static Future<ApiResponse<void>> resetAntrian() async {
    try {
      ...
      final response = await http
          .post(
            Uri.parse(
                '${AppConstants.baseUrl}${AppConstants.antrianEndpoint}/reset'),
            headers: headers,
          )
          .timeout(const Duration(seconds: 15));
  ...
  /// DELETE /antrian/{id} (Mahasiswa, untuk riwayat)
  static Future<void> deleteAntrian(int id) async {
    try {
      final headers = await _authHeaders;
      final response = await http
          .delete(
      ...
      throw ApiException('Gagal menghapus riwayat antrian.');
    }
  }
}
